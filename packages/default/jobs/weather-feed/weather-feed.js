...
module.exports = {
  ...
  onRun: function (config, dependencies, jobCallback) {
    ...
    /*
     ...
     It is a good idea to cover this with unit tests as well (see test/weather-feed file)
     ...
     Have a look at test/weather-feed for an example of how to unit tests this easily by mocking easyRequest calls

     */

    dependencies.easyRequest.HTML(config.url, function (err, json) {
      // var xmlObj = $.load(xml, {
      ...
      // var x2j = new x2js();
      // var entries = [];
      // var result1 = x2j.xml2js(xml);
      // _.each(result1.feed.entry, function(elem){
      //   entries.push({
      //     title: elem.title,
      //     summary: elem.summary.toString()
      //   });
      // });
      //jobCallback(err, {title: config.widgetTitle, entries: entries});
      jobCallback(err, {title: config.widgetTitle, data: json});
