...
module.exports = {
  ...
  onInit: function (config, dependencies) {
    ...
    */
    ...
    this.sendRequest = function (config, dependencies, jobCallback) {
      dependencies.easyRequest.HTML(config.url, function (err, data) {
        // logger.trace(data);
        json = JSON.parse(data);
        if (json.error)
          jobCallback("", {error: json.error.message, code: json.error.code, quote: "We spoke too much today.", author: "Anon"});
        else if (json.success) {
          this.quote = json.contents.quotes[0].quote;
          this.author = json.contents.quotes[0].author;
          this.background = json.contents.quotes[0].background;
          this.lastUpdated = moment();
          jobCallback(err, {quote: this.quote, author: this.author, background: this.background});
        }
        else
          jobCallback(err, {quote: "No one spoke today.", author: "Anon"});
      });
    }
  },
  ...
  onRun: function (config, dependencies, jobCallback) {
    ...
    /*
     ...
     It is a good idea to cover this with unit tests as well (see test/theysaidso file)
     ...
     Have a look at test/theysaidso for an example of how to unit tests this easily by mocking easyRequest calls

     */
    // Cached quote of same day
    if (this.quote && this.lastUpdated)
      if (this.lastUpdated.format('l') === moment().format('l'))
        jobCallback("", {quote: this.quote, author: this.author, background: this.background});
      else {
        this.sendRequest(config, dependencies, jobCallback)
      }
    else {
      this.sendRequest(config, dependencies, jobCallback)
    }
